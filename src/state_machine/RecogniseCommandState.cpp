...
void RecogniseCommandState::enterState()
{
    ...
    TickType_t m_start_time = xTaskGetTickCount();
    m_elapsed_time = 0;
    m_last_audio_position = -1;

    uint32_t free_ram = esp_get_free_heap_size();
    ESP_LOGI("Memory state", "Free ram before connection %d\n", free_ram);

    m_speech_recogniser = new WitAiChunkedUploader(COMMAND_RECOGNITION_ACCESS_KEY);

    ESP_LOGI("Memory state","Ready for action");

    free_ram = esp_get_free_heap_size();
    ESP_LOGI("Memory state","Free ram after connection %d\n", free_ram);
}
bool RecogniseCommandState::run()
{
    if (!m_speech_recogniser || !m_speech_recogniser->connected())
    {
        ...
        ESP_LOGI("Connect state","Error - Attempt to run with no http client");
        return true;
    }
    ...
    if (sample_count > 0)
    {
        ...
        TickType_t current_time = esp_timer_get_time() / 1000;
        ...
        if (m_elapsed_time > 3000)
        {
            ...
            ESP_LOGI("Connect Status","3 seconds has elapsed - finishing recognition request");
...
void RecogniseCommandState::exitState()
{
    ...
    m_speech_recogniser = NULL;
    uint32_t free_ram = esp_get_free_heap_size();
    ESP_LOGI("Memory status","Free ram after request %d\n", free_ram);
}