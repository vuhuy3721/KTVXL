...
WAVFileReader::WAVFileReader(const char *file_name, bool repeat){
    ...
    {   
        ESP_LOGE("WAVFileReader", "ERROR: bit depth %d is not supported please use 16 bit signed integer\n", wav_header.bit_depth );
    }
    if (wav_header.sample_rate != 16000)
    {
        ESP_LOGE("WAVFileReader","ERROR: bit depth %d is not supported please us 16KHz\n", wav_header.sample_rate);
    }
